...
public class AlignToShootPoseCommand extends AlignToPoseCommand {
    ...
    public static Pose2d getTargetPose(){
        Pose2d currentPose = RobotState.isAllianceRed() ? FlippingUtil.flipFieldPose(RobotState.getInstance().getEstimatedPose()) : RobotState.getInstance().getEstimatedPose();
        double middle = DriveConstants.BLUE_HUB_ORIGIN.getY();
        Pose2d flippedUnderTrench = new Pose2d(UNDER_TRENCH.getX(), middle + middle - UNDER_TRENCH.getY(), new Rotation2d());
        ...
        double angle = Math.atan2(closestPose.getY() - middle, closestPose.getX() - DriveConstants.BLUE_HUB_ORIGIN.getX());
        return new Pose2d(closestPose.getX(), closestPose.getY(), new Rotation2d(angle).plus(Rotation2d.kPi));
    }
}
