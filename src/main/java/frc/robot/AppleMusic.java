...
import frc.robot.MasterInfo.RobotType;
import frc.robot.some_stuff_IDK_what.toes.FootMeasurements;
import frc.robot.we_should_DELETE_these.Balls;
...
public class AppleMusic {
  ...
  private AppleMusic() {
    ...
    driveSimulation =
        new SwerveDriveSimulation(
            FootMeasurements.mapleSimConfig, MyPlaylist.getInstance().getEstimatedPose());
    arena.addDriveTrainSimulation(driveSimulation);

    SimulatedArena.overrideInstance(arena);

    // init shooter sim
    fuelSim = new Balls("FieldSimulation");
    fuelSim.registerRobot(
        FootMeasurements.mapleSimConfig.bumperWidthY, // from front to back in meters
        FootMeasurements.mapleSimConfig.bumperLengthX, // from left to right in meters
    ...
    double halfLength = FootMeasurements.mapleSimConfig.bumperLengthX.in(Meters) / 2.0;
    double halfWidth = FootMeasurements.mapleSimConfig.bumperWidthY.in(Meters) / 2.0;
  ...
  private static AppleMusic theOneAndOnly = null;

  public static AppleMusic getInstance() {
    if (MasterInfo.getRobotType() != RobotType.UNREAL) {
      // idiot proofing
      System.out.println(
          "WARNING: YOU ARE TRYING TO ACCESS ROBOT SIM STATE FROM AN ACTUAL ROBOT -- THIS IS A CODE"
              + " ERROR");
    }
    if (theOneAndOnly == null) theOneAndOnly = new AppleMusic();
  ...
  private Balls fuelSim;

  public Balls getFuelSim() {
    return fuelSim;
  }
