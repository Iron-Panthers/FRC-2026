  ...
  public Robot() {
    ...
    switch (Constants.getRobotMode()) {
      ...
      case SIM:
        ...
        Logger.addDataReceiver(new NT4Publisher());
        // When running headlessly for AI tooling, also write a .wpilog for log analysis.
        // Activated via -Dai.logging=true (set by ./gradlew simulateJava -Pai.logging=true).
        if (Boolean.getBoolean("ai.logging")) {
          ...
          // Headless sim starts disabled with no GUI to click Enable.
          // Enable in autonomous mode so autonomousInit() fires and runs the auto.
          boolean runAuto = System.getProperty("ai.auto.name") != null;
          new Thread(
                  () -> {
                    try {
                      Thread.sleep(2000); // Wait for RobotContainer init to complete
