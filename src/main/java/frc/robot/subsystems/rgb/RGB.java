...
public class RGB extends SubsystemBase {
  public static enum RGBMessages {
    CRITICAL_NETWORK_FAILURE(
        new RGBMessage(
            LEDPattern.solid(Color.kOrange).blink(Seconds.of(1/8)), MessagePriority.A_CRITICAL_NETWORK_FAILURE, true)),
    MISSING_CAN_DEVICE(
        new RGBMessage(LEDPattern.solid(Color.kRed), MessagePriority.B_MISSING_CAN_DEVICE, true)),
    CLIMB(new RGBMessage(LEDPattern.solid(Color.kRed).blink(Seconds.of(1/8)), MessagePriority.C_CLIMB, true)),
    READY_TO_INTAKE(
        new RGBMessage(LEDPattern.solid(Color.kBlue).blink(Seconds.of(1/8)), MessagePriority.D_READY_TO_INTAKE, true)),
    L2(new RGBMessage(LEDPattern.solid(Color.kOrange).blink(Seconds.of(1/8)), MessagePriority.E_L2, true)),
    L3(new RGBMessage(LEDPattern.solid(Color.kYellow).blink(Seconds.of(1/8)), MessagePriority.F_L3, true)),
    L4(new RGBMessage(LEDPattern.solid(Color.kBlue).blink(Seconds.of(1/8)), MessagePriority.G_L4, true)),
    L1(new RGBMessage(LEDPattern.solid(Color.kRed).blink(Seconds.of(1/8)), MessagePriority.H_L1, true)),
    CORAL_DETECTED(
        new RGBMessage(LEDPattern.solid(Color.kGreen).breathe(Seconds.of(2)), MessagePriority.I_CORAL_DETECTED, true)),
    DEFAULT(new RGBMessage(LEDPattern.rainbow(255, 51).scrollAtAbsoluteSpeed(MetersPerSecond.of(0.5), Meters.of(0.01)), MessagePriority.J_DEFAULT, false));
